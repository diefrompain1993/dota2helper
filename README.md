...
4. **Запустите приложение** из папки проекта:
   ...
   * скачивание единого архива ассетов (герои + предметы) с GitHub и распаковка в `assets/heroes` и `assets/items` при отсутствии локальных файлов;
   ...
   * запуск GSI-сервера на `0.0.0.0:4000` и открытие окна Tkinter.
   > ⚠️ Первый запуск скачивает один ZIP (~несколько мегабайт) с иконками; повторные запуски используют локальный кэш и не обращаются к сети. Прогресс и возможные ошибки загрузки отражаются в логах INFO/ERROR.
