# dota2helper

Прототип «ИИ-подсказчика предметов для Dota 2» на Python.

- [Архитектура (GSI + OCR)](docs/architecture.md)
- [Техническое задание](docs/technical_spec.md)

## Соответствие чек-листу

* **GSI-сервер** — реализован на Flask, принимает POST `/gsi`, хранит героя и предметы в памяти (`gsi_server.py`).
* **Захват экрана** — делает снимки по именованным регионам из `config/regions.json` с помощью `mss` (`screen_capture.py`).
* **OCR/распознавание иконок** — template matching для врагов и их предметов, сетка слотов фиксированная (`ocr_recognition.py`, папки `assets/heroes`, `assets/items`).
* **Рекомендации** — rule-based логика по тегам героев и контр-предметам (`recommender.py`).
* **UI** — окно на Tkinter, выводит героя, предметы, врагов, рекомендации с автообновлением (`ui.py`).
* **Главный цикл** — поднимает GSI, оркестрирует захват, распознавание, рекомендации и UI (`main.py`).

## Полностью автоматический запуск (Windows)

1. **Установите Python 3.10+** и добавьте его в `PATH`.
2. **Склонируйте проект** или скачайте архив с кодом, затем откройте папку с `README.md` в терминале.
3. **Установите зависимости** (включая Tkinter, если он не входит в вашу сборку Python):
   ```bash
   pip install flask mss opencv-python numpy pyautogui
   ```
4. **Запустите приложение** из папки проекта:
   ```bash
   python main.py
   ```
   При старте автоматически выполняется:
   * создание/копирование GSI-конфига `dota2helper.cfg` в `.../game/dota/cfg/gamestate_integration` (если найден Steam);
   * автокалибровка `config/regions.json` под ваше текущее разрешение экрана;
   * загрузка `data/heroes.json` и `data/items.json` из OpenDota;
   * скачивание всех иконок героев и предметов из CDN Valve в `assets/heroes` и `assets/items` (догружает недостающие);
   * загрузка шаблонов в OCR и запуск фонового цикла с захватом, распознаванием и рекомендациями каждые ~0.75 с;
   * запуск GSI-сервера на `0.0.0.0:4000` и открытие окна Tkinter.
   > ⚠️ Первый запуск может занять несколько минут из‑за загрузки всех иконок (200+ героев, 250+ предметов). В логах INFO/DEBUG видно прогресс по скачанным файлам; повторные запуски используют кэш и стартуют быстрее.
5. **В игре нажмите TAB** для отображения вражеских портретов и слотов на экране — OCR использует этот вид.
6. **Смотрите рекомендации** в окне: герой, ваши предметы, распознанные враги и их сборки, предложенные предметы с пояснениями.
7. **Логи**:
   * консоль + файл `logs/app.log` (ротация 5×1 МБ);
   * INFO по умолчанию, детальная диагностика по OCR/захвату — в уровне DEBUG.

### Создание standalone EXE

Для однофайлового запуска без Python-терминала:

```bash
pyinstaller --onefile --windowed main.py ^
  --add-data "assets;assets" ^
  --add-data "config;config" ^
  --add-data "data;data"
```
